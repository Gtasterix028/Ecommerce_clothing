...
public class VendorController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Response> updateVendor(@PathVariable UUID id, @RequestBody Vendor vendor) {
        try {
    ...
    @PatchMapping("/{id}")
    public ResponseEntity<Response> patchVendorById(@PathVariable UUID id, @RequestBody Vendor vendor) {
        try {
            Vendor updatedVendor = vendorService.patchVendorById(id, vendor);
            return ResponseEntity.ok(new Response("Vendor updated successfully", updatedVendor, false));
        } catch (ValidationException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new Response(e.getMessage(), "An error occurred", true));
        } catch (Exception e) {
